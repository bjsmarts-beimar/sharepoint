...
jQuery(document).ready(function () {
    ...
    if ( Create !== 'Yes')
    {
        jQuery.ajax  
        ({  
            url: _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('exceptions')/items?$select=Title,ID,Comments&$filter=Revision_x0020_Id eq " + RevisionId,  
            type: "GET",  
            headers:  
            {  
...
{  
    jQuery.ajax  
    ({  
        url: _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('revisions')/items?$select=Title,Link,Created,Revision_x0020_Id,ID&$filter=Revision_x0020_Id eq " + RevisionID,  
        type: "GET",  
        headers:  
        {  
            "Accept": "application/json;odata=verbose",  
...
function Cancel()
{
    ...
    var RevisionId = getUrlParameter('RevisionId');
    window.location = serverUrl + "/SitePages/process.aspx?RevisionId=" + RevisionId;
}
...
function UpdateExceptionForm(ExceptionId)
{
    ...
    $.ajax  
    ({  
        url: _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('Exceptions')/items(" + ExceptionId + ")", // list item ID  
        type: "POST",  
...
function UpdateRevision(RevisionId)
{
    ...
    $.ajax  
    ({  
        url: _spPageContextInfo.webAbsoluteUrl + "/_api/web/lists/GetByTitle('Revisions')/items(" + RevisionId + ")", // list item ID  
        type: "POST",  
        data: JSON.stringify(data),  
        headers:  
        {  
